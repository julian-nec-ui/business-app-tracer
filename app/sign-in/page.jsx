      ...
      <Card className="w-full max-w-lg border-gray-200 shadow-lg">
        ...
        <form className="space-y-4" onSubmit={handleSubmit}>
          <CardContent className="space-y-4" key={key}>
            {error && (
              <div className="p-3 text-sm font-bold rounded-md bg-destructive/10 text-destructive">
                <span className="flex items-center gap-3"><TriangleAlertIcon size={26} />{error}</span>
              </div>
            )}
            <div>
              <Label htmlFor="email" className="mb-2">Email</Label>
              <Input onChange={(e) => handleEnableSubmit(e)} className="mb-2" id="email" name="email" type="email" placeholder="jdoe@example.com" required icon={<MailIcon className="absolute w-4 h-4 transform -translate-y-1/2 pointer-events-none top-3/7 right-2" />} />
            </div>
            <div>
          ...
          <CardFooter className="flex flex-col px-6 space-y-4">
            <Button disabled={!enableSubmit || loading} className="w-full bg-blue-500 hover:bg-blue-600" type="submit">
              {loading ? "Signing in..." : "Sign In"}

              {loading && (
                <svg className="w-16 h-16 text-red-600 animate-spin" viewBox="0 0 64 64" fill="none"
            ...
            <p className="text-sm text-center text-gray-600">
              Don't have an account? <Link href="/sign-up" className="font-medium text-blue-500 hover:underline">Sign Up</Link>
            </p>
          </CardFooter>
        </form>
      </Card>
